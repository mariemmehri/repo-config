...
Combinées, ces trois policies isolent le trafic est-ouest du namespace `staging` : seul `hr-frontend → hr-backend` et le DNS sortant vers `kube-dns` (depuis `hr-backend`/`hr-frontend`) sont permis, tout le reste (y compris un pod compromis ailleurs dans le cluster qui tenterait d'atteindre `hr-backend` directement, ou une exfiltration DNS vers un serveur externe) est bloqué par le deny-all par défaut.
