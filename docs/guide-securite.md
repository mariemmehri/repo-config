...
**Comment c'est implémenté :** `modules/gke/main.tf` active `network_policy { enabled = true, provider = "CALICO" }` sur le cluster — cela **active le moteur d'application** des NetworkPolicy Kubernetes (sans Calico, les objets `NetworkPolicy` créés seraient silencieusement ignorés par GKE). Le moteur est désormais complété par trois objets `NetworkPolicy` dans `charts/hr-app/templates/`, gouvernés par `.Values.networkPolicy.enabled` (`false` par défaut, `true` dans `values-staging.yaml`) :
...
- `networkpolicy-backend.yaml` : autorise l'ingress vers `hr-backend` uniquement depuis les pods `app: hr-frontend` sur le port applicatif (`.Values.backend.port`), et l'egress uniquement vers `kube-system` en UDP/TCP 53 (résolution DNS).
- `networkpolicy-frontend.yaml` : autorise l'ingress vers `hr-frontend` depuis n'importe quelle source sur `.Values.frontend.port` (le frontend doit rester joignable, y compris via `kubectl port-forward` ou un futur ingress), et l'egress uniquement vers `hr-backend` + DNS.
