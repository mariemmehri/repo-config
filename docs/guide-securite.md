...
- `networkpolicy-backend.yaml` : autorise l'ingress vers `hr-backend` uniquement depuis les pods `app: hr-frontend` sur le port applicatif (`.Values.backend.port`), et l'egress DNS via un `ipBlock` scopé à la ClusterIP `kube-dns` (voir détail ci-dessous).
...
Combinées, ces trois policies isolent le trafic est-ouest du namespace `staging` : seul `hr-frontend → hr-backend` et le DNS sortant vers `kube-dns` sont permis, tout le reste (y compris un pod compromis ailleurs dans le cluster qui tenterait d'atteindre `hr-backend` directement, ou une exfiltration DNS vers un serveur externe) est bloqué par le deny-all par défaut.
