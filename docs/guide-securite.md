...
**Étape intermédiaire abandonnée — `ipBlock` scopé à la ClusterIP `kube-dns` :** avant la solution actuelle, une version utilisait `ipBlock: { cidr: <ClusterIP kube-dns>/32 }` plutôt qu'un selector, avec `networkPolicy.dnsClusterIP` comme valeur de chart portant cette IP. Elle fonctionnait avec Calico (qui sait enforcer un `ipBlock` contre une ClusterIP, contrairement à un `podSelector`/`namespaceSelector`), mais reposait sur une IP de ClusterIP non garantie stable dans le temps (une recréation de cluster peut la changer). Cette approche **n'est plus utilisée** : `networkPolicy.dnsClusterIP` a été retiré du chart (`d9d5a19`), au profit des deux `podSelector`/`namespaceSelector` (`kube-dns` + `node-local-dns`) décrits ci-dessus, une fois la vraie cause racine identifiée (NodeLocal DNSCache, pas Calico vs Cilium). Historique complet des quatre itérations dans [issues-rencontrees.md](issues-rencontrees.md#issue-4--calico-ne-peut-pas-enforcer-une-networkpolicy-egress-contre-une-clusterip-dns-cassé).
