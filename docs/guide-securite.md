...
**Comment c'est implémenté :** `modules/gke/main.tf` active `network_policy { enabled = true, provider = "CALICO" }` sur le cluster — cela **active le moteur d'application** des NetworkPolicy Kubernetes (sans Calico, les objets `NetworkPolicy` créés seraient silencieusement ignorés par GKE). Le moteur est complété par trois objets `NetworkPolicy` dans `charts/hr-app/templates/`, gouvernés par `.Values.networkPolicy.enabled` (`false` par défaut, `true` dans `values-staging.yaml`) :
...
- `networkpolicy-backend.yaml` : autorise l'ingress vers `hr-backend` uniquement depuis les pods `app: hr-frontend` sur le port applicatif (`.Values.backend.port`), et l'egress DNS via un `ipBlock` (voir limite ci-dessous).
...
Combinées, ces trois policies isolent le trafic est-ouest du namespace `staging` : seul `hr-frontend → hr-backend` et le DNS sortant sont permis, tout le reste (y compris un pod compromis ailleurs dans le cluster qui tenterait d'atteindre `hr-backend` directement) est bloqué par le deny-all par défaut.

**⚠️ Limite connue et assumée — egress DNS non scopé à `kube-dns` :** l'egress DNS des deux policies utilise `ipBlock: { cidr: 0.0.0.0/0 }` restreint au port 53 (UDP+TCP), et non un `namespaceSelector`/`podSelector` ciblant précisément les pods `kube-dns`. Raison : les pods résolvent le DNS via la **ClusterIP** du service `kube-dns` (visible dans `/etc/resolv.conf`, ex. `34.118.224.10`), pas directement via l'IP du pod — et **Calico legacy sur GKE (mode iptables, sans Dataplane V2) ne peut pas faire correspondre une règle d'egress `NetworkPolicy` à une destination ClusterIP**, seules les IP de pod sont évaluées correctement. Vérifié en test live : une requête DNS vers l'IP du pod `kube-dns` réussissait avec un `namespaceSelector`+`podSelector` scopé, la même requête vers la ClusterIP échouait avec un timeout systématique. Une migration vers GKE Dataplane V2 (Cilium/eBPF, qui résout correctement le trafic ClusterIP) a été testée puis **abandonnée** — elle nécessite de recréer le cluster GKE en entier, jugé disproportionné pour ce cluster de staging PFE. Conséquence assumée : un pod compromis pourrait exfiltrer des données via DNS tunneling vers un serveur externe sans être bloqué par cette policy (le port 53 est ouvert vers toute IP, pas seulement `kube-dns`). Détail complet de l'investigation : [issues-rencontrees.md](issues-rencontrees.md#issue-4--calico-ne-peut-pas-enforcer-une-networkpolicy-egress-contre-une-clusterip-dns-cassé).
...
- Pas de `securityContext` Kubernetes (`runAsNonRoot`, `readOnlyRootFilesystem`, `allowPrivilegeEscalation: false`) dans les templates Helm.
