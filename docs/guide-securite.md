...
## 🚀 NetworkPolicy (Calico) sur le cluster GKE
...
**Comment c'est implémenté :** `modules/gke/main.tf` active `network_policy { enabled = true, provider = "CALICO" }` sur le cluster — cela **active le moteur d'application** des NetworkPolicy Kubernetes (sans Calico, les objets `NetworkPolicy` créés seraient silencieusement ignorés par GKE). Le moteur est complété par quatre objets `NetworkPolicy` dans `charts/hr-app/templates/`, gouvernés par `.Values.networkPolicy.enabled` (`true` dans `values.yaml` et `values-staging.yaml`) :
...
- `networkpolicy-backend.yaml` : autorise l'ingress vers `hr-backend` uniquement depuis les pods `app: hr-frontend` sur le port applicatif (`.Values.backend.port`), et l'egress DNS via un `ipBlock` scopé à la ClusterIP `kube-dns` (voir détail ci-dessous) plus, si `.Values.postgres.enabled`, l'egress vers `app: postgres` sur `.Values.postgres.port`.
- `networkpolicy-frontend.yaml` : autorise l'ingress vers `hr-frontend` depuis n'importe quelle source sur `.Values.frontend.port` (le frontend doit rester joignable, y compris via `kubectl port-forward` ou un futur ingress), et l'egress uniquement vers `hr-backend` + DNS (même règle `ipBlock`).
- `networkpolicy-postgres.yaml` (conditionnel à `.Values.postgres.enabled` en plus de `.Values.networkPolicy.enabled`) : autorise l'ingress vers le pod `app: postgres` uniquement depuis `app: hr-backend` sur `.Values.postgres.port`, et **aucun egress** (`egress: []`) — Postgres n'a besoin d'initier aucune connexion sortante, y compris DNS, donc le deny-all par défaut s'applique tel quel côté egress.

Combinées, ces quatre policies isolent le trafic est-ouest du namespace `staging` : seul `hr-frontend → hr-backend → postgres` et le DNS sortant vers `kube-dns` (depuis `hr-backend`/`hr-frontend`) sont permis, tout le reste (y compris un pod compromis ailleurs dans le cluster qui tenterait d'atteindre `hr-backend` ou `postgres` directement, ou une exfiltration DNS vers un serveur externe) est bloqué par le deny-all par défaut.
...
- Le `StatefulSet` `postgres` (`templates/statefulset-postgres.yaml`) fait exception : il déclare un `securityContext` de pod (`runAsNonRoot: true`, `runAsUser`/`runAsGroup`/`fsGroup: 999` — l'UID/GID de l'utilisateur `postgres` dans l'image officielle `postgres:16-alpine`, `seccompProfile: RuntimeDefault`) et un `securityContext` de conteneur (`allowPrivilegeEscalation: false`, `capabilities.drop: ["ALL"]`, `readOnlyRootFilesystem: true`). Le rootfs en lecture seule fonctionne car les trois seuls chemins où Postgres écrit sont montés en volumes séparés : `/var/lib/postgresql/data` (le `PersistentVolumeClaim`, avec `PGDATA` pointé sur un sous-répertoire `pgdata` pour éviter un `lost+found` non vide sur un volume neuf), `/tmp` et `/var/run/postgresql` (deux `emptyDir`). `fsGroup: 999` est ce qui rend le PVC neuf accessible en écriture au groupe `postgres` sans jamais faire tourner l'entrypoint en `root`.
...
- Pas de `securityContext` Kubernetes (`runAsNonRoot`, `readOnlyRootFilesystem`, `allowPrivilegeEscalation: false`) sur les Deployments `hr-backend`/`hr-frontend` — seul le `StatefulSet` `postgres` en déclare un (voir section précédente).
