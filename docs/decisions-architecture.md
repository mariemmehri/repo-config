...
**ArgoCD, pull-based.** Le job `docker-build-push` de `repo-app/.github/workflows/ci.yml` ne détient **aucun accès `container.developer`/credentials GKE au-delà de `roles/container.developer`** dans les rôles accordés à `sa-github-actions` (`backend-config/wif.tf`) — et surtout, ne fait jamais de `kubectl`/`helm upgrade` lui-même. Il se contente d'un `git push` vers `repo-config`. Ce sont les Applications ArgoCD `hr-dev`/`hr-staging`/`hr-prod` (`apps/children/{dev,staging,prod}.yaml`), tournant *dans* le cluster, qui détectent le nouveau commit et synchronisent (`hr-prod` sur sync manuel uniquement, voir [guide-argocd-gitops.md](guide-argocd-gitops.md)).

### ✅ Conséquences

- **Aucun credential d'écriture sur le cluster ne quitte jamais le cluster.** Le pipeline CI de `repo-app` n'a besoin d'aucun accès `kubectl`/GKE en écriture pour déployer — seul un token GitHub App de courte durée (écriture sur `repo-config`, scope limité à ce seul dépôt, expire en ~1h — a remplacé un `GH_PAT` longue durée plus tôt dans le projet) et les rôles GCP limités (`artifactregistry.writer`, `container.developer`, `storage.objectViewer`) suffisent.
