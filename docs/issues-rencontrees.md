...
Une migration vers GKE Dataplane V2 (`datapath_provider = "ADVANCED_DATAPATH"` dans `modules/gke/main.tf`, à la place de `network_policy { enabled = true, provider = "CALICO" }`) a été préparée : Cilium (eBPF) résout correctement le trafic vers une ClusterIP jusqu'à ses pods backend avant d'appliquer la policy, contrairement à Calico/iptables. Ce changement nécessite de **recréer le cluster GKE** (le `datapath_provider` ne se bascule pas en place) via `destroy-staging` puis `apply`, et déclenchait aussi un nouveau finding Checkov (`CKV_GCP_12`, faux positif — le check ne reconnaît que le bloc `network_policy{enabled=true}` de Calico). Décision à l'époque : **revert complet** de cette migration (`git reset --hard` sur `repo-infrastructure` + force-push) — trop de disruption pour ce cluster de staging PFE pour le bénéfice obtenu. **Mise à jour :** cette décision a depuis été inversée — la migration vers Dataplane V2 (`datapath_provider = "ADVANCED_DATAPATH"`) a été refaite et est en place aujourd'hui (voir `docs/guide-securite.md`). Comme la section suivante le montre, ça n'a pas suffi à corriger le DNS : la vraie cause racine n'avait rien à voir avec Calico vs Cilium.
...
Conclusion de l'investigation initiale à revoir : l'échec du `namespaceSelector`/`podSelector` ne prouvait pas que **tout** ciblage de la ClusterIP échoue avec Calico legacy — seulement que ce mécanisme de *selector* ne sait pas relier un pod matché au trafic post-DNAT. Un `ipBlock`, lui, matche directement l'IP de destination du paquet (avant DNAT côté kube-proxy), donc cibler la ClusterIP via `ipBlock` fonctionne. **Cette conclusion elle-même s'est révélée incomplète — voir la mise à jour ci-dessous.**

**Mise à jour (v3) — le vrai coupable était NodeLocal DNSCache, pas Calico/Cilium**

Une fois Dataplane V2 réellement en place (voir la note plus haut), la règle `ipBlock` scopée à la ClusterIP a de nouveau cassé le DNS de façon intermittente. Un premier correctif (`ee72048`) est repassé sur un `namespaceSelector`/`podSelector` visant `kube-dns`, en supposant que Cilium (contrairement à Calico) saurait résoudre une ClusterIP vers ses pods backing avant d'enforcer la policy. Vérification live : ça ne fonctionnait toujours pas (`88aae5f`), donc retour à l'`ipBlock` scopé — sans changer le symptôme de fond.

Le vrai diagnostic est venu de `cilium-dbg monitor --type drop` (`d9d5a19`) : les requêtes DNS des pods `hr-backend`/`hr-frontend` n'atteignent **jamais** `kube-dns` ni sa ClusterIP — le **NodeLocal DNSCache** de GKE intercepte le trafic port 53 et répond depuis un pod `node-local-dns` local au nœud à la place. Ni l'ancien selector `kube-dns`-only, ni l'`ipBlock` ne l'autorisaient, d'où les "Policy denied ... k8s-app=node-local-dns" dans les drops Cilium. Correctif final : deux règles `namespaceSelector`+`podSelector` dans `kube-system`, une pour `k8s-app: kube-dns`, une pour `k8s-app: node-local-dns`. `networkPolicy.dnsClusterIP` est redevenu inutile et a été retiré de `values.yaml` et des trois overlays.

**Leçon :** le problème n'a jamais été Calico vs Cilium/Dataplane V2 — c'est resté un red herring sur toute la durée de l'investigation. La vraie cause était une couche GKE (NodeLocal DNSCache) invisible depuis la définition de la NetworkPolicy elle-même ; seule l'observation live du chemin réel du paquet (`cilium-dbg monitor --type drop`) l'a révélée. Voir [guide-securite.md](guide-securite.md) pour l'état à jour de cette mesure de sécurité.
