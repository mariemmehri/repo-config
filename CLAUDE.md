...
- Ingress `/metadata/annotations` and `/metadata/finalizers` (added `4e4784d`, same commit that wired up the native GCE Ingress) — GKE's GCE Ingress controller writes its own annotations (e.g. backend-config/NEG status bookkeeping) and a finalizer onto the `Ingress` object after creation; without this exception every reconcile loop would flag `hr-ingress` `OutOfSync` and `selfHeal` would fight the controller.
...
The GCE Ingress wiring (commits `4e4784d`, `b04973b`, both on `2026-07-30`) touches several files at once, easy to miss if you only read `ingress.yaml`: `service-backend.yaml`/`service-frontend.yaml` both carry `cloud.google.com/neg: '{"ingress": true}'` (container-native load balancing — the LB targets pod IPs directly via a Network Endpoint Group, not the Service's ClusterIP/kube-proxy path); `networkpolicy-backend.yaml` gained a second `ingress` rule allowing `130.211.0.0/22` and `35.191.0.0/16` (Google's health-check prober ranges) into `hr-backend` on `backend.port` — without it the GCE health check can never pass and every `/api` request 502s at the load balancer even though the pod itself is Ready. `networkpolicy-frontend.yaml` needed no equivalent addition — its `ingress` rule already has no `from:` selector (open to any source on `frontend.port`), so the prober ranges were already allowed. All three `apps/children/{dev,staging,prod}.yaml` also gained the Ingress `ignoreDifferences` entry noted above in the same change.
